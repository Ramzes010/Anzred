function updateRange() {
    ...
    // Map 0-100 range to 1-24 months
    const mappedMin = Math.round((min / 100) * 23) + 1;
    const mappedMax = Math.round((max / 100) * 23) + 1;
  
    minValue.textContent = `От ${mappedMin} месяцев`;
    maxValue.textContent = `${mappedMax} месяцев`;
